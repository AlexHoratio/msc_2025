...
for (filename in list.files("data/humann/renormalized/prodigal")) {
  if (grepl("pathabundance", filename)) {
    if (length(humann_prodigal) == 0) {
      humann_prodigal <- read_tsv(paste0("data/humann/renormalized/prodigal/", filename), col_names = TRUE)
      ...
      pathabundance <- read_tsv(paste0("data/humann/renormalized/prodigal/", filename), col_names = TRUE)
