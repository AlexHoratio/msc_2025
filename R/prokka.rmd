...
cog_def <- read_tsv("data/loma/RAMAN_PROKKA/COG/cog-24.def.tab", col_names = FALSE)
colnames(cog_def) <- c("COG", "Function", "Name", "Gene", "Pathway", "PubMed ID", "PDB ID")

cog_fun <- read_tsv("data/loma/RAMAN_PROKKA/COG/cog-24.fun.tab", col_names = FALSE) %>% na.omit
...
prokka_cog_function_sums <- ddply(prokka_cog_def_sums[, -7], "Description", numcolwise(sum)) %>%
  transform(R003 = R003/as.numeric(read_statistics[1, 10])) %>%
  transform(R004 = R004/as.numeric(read_statistics[2, 10])) %>%
  transform(R005 = R005/as.numeric(read_statistics[3, 10])) %>%
  transform(R007 = R007/as.numeric(read_statistics[4, 10])) %>%
  transform(R012 = R012/as.numeric(read_statistics[5, 10])) %>%
  transform(R018 = R018/as.numeric(read_statistics[6, 10]))
...
prokka_cog_function_sums_percentage <- prokka_cog_function_sums_percentage[, -c(8)] %>%
...
prokka_cog_function_sums_boxplot <- ggplot(prokka_cog_function_sums_percentage, aes(x=Description, y = value, colour=name)) + 
  ...
  coord_flip() +
  xlab("") +
  ylab("Percentage of All Annotations (Across Samples, Normalized by Total Binned b.p.)") +
