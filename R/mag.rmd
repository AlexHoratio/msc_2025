...
remainder <- aggregate(. ~ Assembler+SampleName, all_species_proportions[all_species_proportions$`Relative Abundance` <= 3,][, -c(1, 5)], sum) %>%
  add_column(Species=c("<3% Species")) %>%
  add_column(Approach = c("NA"))

all_species_proportions <- all_species_proportions %>%
  filter(`Relative Abundance` > c(3)) %>%
  rbind(remainder)
...
ggsave("graphs/nfmag/compare_species.png", coverm_species, units = "px", width = 2900, height = 1800)
coverm_species
...
flye_species_matrix <- species_matrix[species_matrix$Assembler == "FLYE", ] %>%
  filter_at(vars(starts_width("R")), any_vars(.>1))
